...
def index(request):
    ...
    for d in sorted(set([i.time[:10] for i in orders])):
        for j in orders:
            if d == j.time[:10]:
                percent_change_for_day[d] = round(sum([round(100*((float(orders[i].all_cost) / float(orders[i+1].all_cost))-1.0015), 2) for i in range(0, len(orders), 2) if orders[i].time[:10] == d]), 2)
