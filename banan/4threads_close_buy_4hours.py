                ...
                if -4.1 > res > -20:
                    ...
                    if name_cript_check == top:
                        ...
                        while open_position:
                            ...
                            high_k_now_price = round(data_token.high_price[-1] / data_token.close_price[-1] * 100 - 100,
                                                     2)
                                ...
                                for i in all_work_crypt[1:round(len(all_work_crypt))]:
                                    ...
                                    if res_now < 0 and res_past < 0.6 and last_time - start_time < 12000 and i[0] not in new_alg_crypto_work_end:
                                        ...
                                        while open_position:
                                            ...
                                            all_orders = pd.DataFrame(client.get_all_orders(symbol=i[0]),
                                                                      columns=["orderId", "type", "side", "price",
                                                                               "status"])
                                            ...
                                            try:
                                                x = Decimal(str(round((buyprice / 100) * sell_pr,
                                                                      max([len(f'{i:.15f}'.rstrip("0").split(".")[1])
                                                                           for i
                                                                           in
                                                                           data_token[0][-5:]]))))
