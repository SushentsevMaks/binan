...
def top_coin(trading_pairs: list):
    for name_cript_check in trading_pairs:
        try:
            ...
            volume_per_5h: float = sum([int(i * data_token.high_price[-1]) for i in data_token.volume[:-7]]) / len(data_token.volume[:-7])
            ...
            if -3.5 > res:
                ...
                if res_k_low < 350:
                    telebot.TeleBot(telega_token).send_message(chat_id, f"{name_cript_check} не прошел проверку по хвосту свечи")
                    break
                ...
                while time.localtime(start_time_check).tm_min % 15 != 14 or time.localtime(start_time_check).tm_sec < 58:
                    start_time_check = time.time()
                    time.sleep(1)
                ...
                top = sorted([[i[0], i[1] + i[2] + i[3], i[5], i[6]] for i in itog], key=lambda x: x[1])[0][0]
                all_work_crypt = sorted([[i[0], i[1] + i[2] + i[3], i[5], i[6]] for i in itog], key=lambda x: x[1])
                ...
                if name_cript_check == top:
                    ...
                    while open_position:
                        ...
                        if float(sell_qty) < 0.1 and len(all_orders[all_orders.isin(["NEW"]).any(axis=1)]) == 0:
                            ...
                            message = f"СДЕЛКА ЗАВЕРШЕНА - {name_cript_check}\n" \
                                      ...
                                      f"\n" \
                                      f"количество крипты на балансе: {sell_qty}"
                            bot.send_message(chat_id, message)

                        if buyprice * 0.993 > data_token.close_price[-1]:
                    ...
                    try:
                        ...
                        telebot.TeleBot(telega_token).send_message(chat_id, f"количество крипты на балансе: {sell_qty}")
